...
import {addShow, getAllShows, paginateShows, removeById, updateBySlug} from 'database/services/show.service';
...
handler.patch(
    ...
    async (req, res) => {
        try {
            ...
            const {body} = req;
            const slug = body.current;
            const show = await updateBySlug(slug, body.data);
            res.status(200).json(show);
        } catch (error) {
            res.status(400).json({message: error.message});
        }
