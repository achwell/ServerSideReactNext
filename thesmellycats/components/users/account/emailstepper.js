    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            newemail: Yup.string()
                ...
                .email('This is not a valid email')
                .test('match', 'Please check your email', (newemail) => newemail !== user.data.email)
        }),
        onSubmit: (values) => {
            setLoading(true)
            console.log({values})

            axios.patch('/api/users', {email: values.newemail})
                .then(response => {
                    dispatch(userSignOut("Done !! Log in again with your new email"))
                    router.push("/")
                })
                .catch(error => {
    ...
    return (
        <>
            <form className="mt-3 article_form" style={{maxWidth: "250px"}}>
                ...
                <Button className="mb-3" variant="contained" color="primary" onClick={openModal}>Edit edit mail</Button>
            </form>
                        ...
                        {activeStep === 2 ?
                            <div className="form-group">
                                {loading ?
                                    ...
                                    :
                                    <>
                                        <Alert variant="warning">Remember, you will be signed out and you need to sign
                                            in with your new email.</Alert>
                                        <Button className="mt-3" variant="contained" color="primary"
                                                onClick={formik.submitForm}>Edit email</Button>
                                        {backsBtn()}
                                    </>
                                }
                            </div>
